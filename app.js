...
function getTagColors(tag) {
  ...
  const PRESET = {
    ...
    '云降水物理': 'hsl(190 90% 40%)',
    '卫星遥感': 'hsl(160 70% 40%)',
    '气溶胶-云相互作用': 'hsl(30 85% 45%)',
    '大气动力过程': 'hsl(340 75% 45%)',
    '人工智能+气象': 'hsl(270 75% 50%)',
    '雷达遥感': 'hsl(200 75% 42%)',
    '激光雷达遥感': 'hsl(120 65% 40%)',
    '大气遥感探测': 'hsl(50 80% 42%)',
    '其他': 'hsl(220 14% 46%)',
  };
...
async function populateFilters() {
  ...
  // tags -> 渲染为按钮
  // 使用全部预定义标签（即使DB中暂无），确保新标签始终可见
  let tags = ['雷达气象', '云降水物理', '卫星遥感', '气溶胶-云相互作用', '大气动力过程', '人工智能+气象', '雷达遥感', '激光雷达遥感', '大气遥感探测', '其他'];
  // types -> 渲染为下拉菜单
  ...
  // 排序规则：雷达气象/云降水物理最前，其他最后，其余按拼音/字母序
  tags.sort((a, b) => {
    if (a === '雷达气象') return -1;
    if (b === '雷达气象') return 1;
    if (a === '云降水物理') return -2;
...
function bindEvents() {
  ...
  document.getElementById('list').addEventListener('click', (e) => {
    ...
    const item = _lastResults.find(r => String(r.uid) === uid);
    if (item) {
      openDetail(item);
    }
  });
...
function openDetail(itemData) {
  _detailData = itemData;
  const overlay = document.getElementById('detail-overlay');
  if (!overlay) return;

  document.getElementById('detail-title').textContent = itemData.title || '(无标题)';

  // Tags
  const tagsEl = document.getElementById('detail-tags');
  const tags = parseTags(itemData.topic_tag);
  tagsEl.innerHTML = tags.length ? tags.map(t => `<span class="tag-chip" style="--tag-bg:${getTagColors(t).active}">${escapeHtml(t)}</span>`).join('') : '';

  // Meta
  document.getElementById('detail-journal').textContent = itemData.journal ? `期刊：${escapeHtml(itemData.journal)}` : '';
  document.getElementById('detail-date').textContent = itemData.pub_date ? `发表日期：${itemData.pub_date}` : '';

  // DOI
  const doiEl = document.getElementById('detail-doi');
  const doi = itemData.doi || '';
  doiEl.innerHTML = doi ? `<a href="https://doi.org/${escapeHtml(doi)}" target="_blank">DOI: ${escapeHtml(doi)}</a>` : '';

  // Abstract
  const absEl = document.getElementById('detail-abstract');
  const absText = state.bilingual
    ? (itemData.abstract_cn || itemData.abstract_en || '')
    : (itemData.abstract_en || itemData.abstract_cn || '');
  absEl.textContent = absText || '(无摘要)';

  // Link
  const linkEl = document.getElementById('detail-link');
  linkEl.href = itemData.article_url || itemData.doi ? `https://doi.org/${itemData.doi}` : '#';
  ...
  const url = itemData.article_url || (doi ? 'https://doi.org/' + doi : '');
  const readCmd = `/read 《${title}》\n期刊：${journal}\nDOI：${doi}\n链接：${url}\n摘要：${abstract.substring(0, 800)}`;
  document.getElementById('detail-read-command').value = readCmd;
  document.getElementById('detail-read-area').hidden = true;
  document.getElementById('detail-copy-msg').hidden = true;
...
document.addEventListener('click', (e) => {
  ...
  if (e.target === overlay || e.target.closest('#detail-close')) {
    closeDetail();
  }
});
...
// 精读按钮：展开精读指令区域
document.addEventListener('click', (e) => {
  const btn = e.target.closest('#detail-read-btn');
  if (!btn || !_detailData) return;
  const area = document.getElementById('detail-read-area');
  if (area) area.hidden = false;
});

// 复制精读指令
document.addEventListener('click', (e) => {
  const btn = e.target.closest('#detail-copy-cmd');
  if (!btn) return;
  const textarea = document.getElementById('detail-read-command');
  if (!textarea) return;
  textarea.select();
  navigator.clipboard.writeText(textarea.value).then(() => {
    const msg = document.getElementById('detail-copy-msg');
...
function buildWhere() {
  ...
  // tag：AND - 选中多个 tag 时取交集，需要文章同时包含所有选中的 tag
